  ...
  return (
    <section className="docs-section">
      ...
      <p className="docs-paragraph">
        A powerful wrapper component to add entrance and exit animations to any child element using Framer Motion presets.
      </p>

      {/* Basic Presets */}
      <h2 className="docs-category-subtitle">Animation Presets</h2>
      <p className="docs-paragraph">
        Choose from a variety of built-in presets like fade, scale, slide, bounce, and flip.
      </p>
      <div className="docs-showcase-card h-auto p-8 flex flex-col items-center gap-12">
        <button onClick={replay} className="docs-button docs-button-secondary mb-4">Replay Animations</button>

        <div key={key} className="grid grid-cols-2 md:grid-cols-3 gap-8 w-full max-w-2xl">
          <div className="flex flex-col items-center gap-2">
            <AnimatedContent preset="fade" duration={0.8}>
              <div className="w-20 h-20 bg-primary/20 rounded-lg flex items-center justify-center border border-primary/30 font-bold">Fade</div>
            </AnimatedContent>
            <span className="text-xs text-neutral-400">fade</span>
          </div>
          <div className="flex flex-col items-center gap-2">
            <AnimatedContent preset="scale" duration={0.8} delay={0.2}>
              <div className="w-20 h-20 bg-primary/20 rounded-lg flex items-center justify-center border border-primary/30 font-bold">Scale</div>
            </AnimatedContent>
            <span className="text-xs text-neutral-400">scale</span>
          </div>
          <div className="flex flex-col items-center gap-2">
            <AnimatedContent preset="slideUp" duration={0.8} delay={0.4}>
              <div className="w-20 h-20 bg-primary/20 rounded-lg flex items-center justify-center border border-primary/30 font-bold">Slide Up</div>
            </AnimatedContent>
            <span className="text-xs text-neutral-400">slideUp</span>
          </div>
          <div className="flex flex-col items-center gap-2">
            <AnimatedContent preset="bounce" duration={1} delay={0.6}>
              <div className="w-20 h-20 bg-primary/20 rounded-lg flex items-center justify-center border border-primary/30 font-bold">Bounce</div>
            </AnimatedContent>
            <span className="text-xs text-neutral-400">bounce</span>
          </div>
          <div className="flex flex-col items-center gap-2">
            <AnimatedContent preset="flip" duration={0.8} delay={0.8}>
              <div className="w-20 h-20 bg-primary/20 rounded-lg flex items-center justify-center border border-primary/30 font-bold">Flip</div>
            </AnimatedContent>
            <span className="text-xs text-neutral-400">flip</span>
          </div>
          <div className="flex flex-col items-center gap-2">
            <AnimatedContent preset="zoom" duration={0.8} delay={1}>
              <div className="w-20 h-20 bg-primary/20 rounded-lg flex items-center justify-center border border-primary/30 font-bold">Zoom</div>
            </AnimatedContent>
...
<AnimatedContent preset="bounce" delay={0.5} duration={1}>
  <div>Bouncy content</div>
      ...
      <div className="docs-showcase-card h-[250px] flex items-center justify-center gap-12">
        <div key={`ease-${key}`} className="flex gap-8">
          <AnimatedContent preset="slideRight" ease="backOut" duration={1}>
            <div className="px-6 py-3 bg-white dark:bg-neutral-800 rounded-lg shadow-lg border border-neutral-200 dark:border-neutral-700 font-bold">Back Out</div>
          </AnimatedContent>
          <AnimatedContent preset="slideRight" ease="anticipate" duration={1} delay={0.3}>
            <div className="px-6 py-3 bg-white dark:bg-neutral-800 rounded-lg shadow-lg border border-neutral-200 dark:border-neutral-700 font-bold">Anticipate</div>
          </AnimatedContent>
        </div>
      </div>

      <CodeBlock code={`<AnimatedContent preset="slideRight" ease="backOut">...</AnimatedContent>`} />
        ...
        <table className="docs-props-table">
          ...
          <tbody>
            <tr>
              ...
              <td><span className="docs-prop-type">AnimationPreset</span></td>
              <td>'fade'</td>
              <td>fade, scale, slideUp, slideDown, slideLeft, slideRight, elastic, bounce, flip, zoom</td>
            </tr>
            <tr>
              <td className="docs-prop-name">ease</td>
              <td><span className="docs-prop-type">AnimationEase</span></td>
              <td>'easeOut'</td>
              <td>linear, easeIn, easeOut, easeInOut, circIn, circOut, backIn, backOut, bounce, elastic, etc.</td>
            </tr>
            <tr>
      ...
      <DocsButtonBar
        ...
        next={{ label: 'Background Gradient', route: '/ui-basics/background-gradient-animation' }}
      />
    </section>
  );
};
