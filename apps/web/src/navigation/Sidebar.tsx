...
import { catalogCategories, componentMap } from "@/catalog/registry";
import type { CatalogCategory } from "@/catalog/registry";
...
const toPascal = (str: string) =>
  str
    ...
    .join("");
const createCatalogPath = (categorySlug: string, itemSlug: string) =>
...
const Sidebar = () => {
  ...
  const findActiveElement = useCallback(() => {
    ...
    for (const category of catalogCategories) {
      const activeItem = category.items.find((item) => {
        return activePath === createCatalogPath(category.slug, item.slug);
      });

      if (activeItem) {
        const isExpanded = expandedCategories[category.slug];
        const subcategoryPath = createCatalogPath(
          category.slug,
          activeItem.slug,
        );
        const subcategoryEl = itemRefs.current[subcategoryPath];
        const categoryHeaderEl = itemRefs.current[`category:${category.slug}`];
  ...
  const handleTransitionNavigation = useCallback(
    async (path: string, itemSlug: string) => {
      if (isTransitioning || location.pathname === path) return;

      setPendingActivePath(path);

      await startTransition(itemSlug, componentMap, () => {
        navigate(path);
        scrollToTop();
        ...
        <div className={SIDEBAR_CONTENT_CLASS_NAME}>
          {catalogCategories.map((cat, index) => (
            <Category
              key={cat.slug}
              category={cat}
              isExpanded={!!expandedCategories[cat.slug]}
              toggleExpanded={() => toggleCategory(cat.slug)}
              onExpandedChange={(expanded) =>
                setCategoryExpanded(cat.slug, expanded)
              }
              location={location}
...
type CategoryProps = {
  ...
  handleClick: () => void;
  handleTransitionNavigation?: (path: string, itemSlug: string) => void;
...
export const Category = memo(function Category({
  ...
  const items = useMemo(() => {
    return category.items.map((item) => {
      const path = createCatalogPath(category.slug, item.slug);
      const activePath = pendingActivePath || location.pathname;
      const favoriteKey = `${toPascal(category.slug)}/${toPascal(item.slug)}`;
      ...
      <div>
        ...
        >
          ...
          {hasActiveItem && !isExpanded && (
            <div className="active-pill-background" />
          )}
          <svg
          ...
          {isExpanded && (
            ...
            >
              <div className="category-content relative pl-4 border-l border-[#392e4e] flex flex-col space-y-2 pb-2">
                {items.map(
                  ({ item, path, isActive, isNew, isUpdated, isFavorited }) => (
                    <Link
                      key={path}
                      to={path}
                      ref={(el) => {
                      ...
                      onClick={(e) => {
                        ...
                        if (handleTransitionNavigation) {
                          handleTransitionNavigation(path, item.slug);
                        } else {
                          handleClick();
                        }
                      }}
                    ...
                    >
                      ...
                      {isActive && <div className="active-pill-background" />}
                    </Link>
                  ),
                )}
              </div>
